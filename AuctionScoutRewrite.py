...
def selectionStrat(itemList):
    itemList.sort()
    lowerQuartile = itemList[int(len(itemList)/4)]
    if lowerQuartile < 20000000:
        return max(0.36 * lowerQuartile, lowerQuartile - 3000000)
    else:
        return 0.4 * lowerQuartile



def writeToIpcFile(uuid, timestamp):
    print("item will be available in", (timestamp / 1000) - time.time() + 19, "seconds")
    ipcData = uuid + "," + str(timestamp + 19000)
...
IGNORE_LIST = ["ATTRIBUTE_SHARD", "FERVOR_HELMET", "FERVOR_CHESTPLATE", "FERVOR_LEGGINGS", "FERVOR_BOOTS", "CRIMSON_HELMET", "CRIMSON_CHESTPLATE", "CRIMSON_LEGGINGS", "CRIMSON_BOOTS",
               ...
               "ROD_OF_THE_SEA", "DYE_CHOCOLATE", "MITHRIL_BELT"]
